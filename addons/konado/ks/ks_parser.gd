...
func _parse_actor() -> KS_AST.ActorNode:
	...
	match action_tok.type:
		KS_Token.Type.KW_SHOW:
			...
			node.state = _read_actor_state_until_at()
			if node.state.is_empty():
				_error("actor show 缺少状态")
				return null

			# 可选 at <position>
			_parse_optional_actor_position(node)
...
func _read_actor_state_until_at() -> String:
	var parts: PackedStringArray = []
	while not _at_line_end() and not _check(KS_Token.Type.KW_AT):
		parts.append(_token_value_to_text(_advance()))
	return "".join(parts)


func _parse_optional_actor_position(node: KS_AST.ActorNode) -> void:
	if _at_line_end() or not _check(KS_Token.Type.KW_AT):
		return
	_advance()  # 跳过 at
	var pos_tok := _expect_any_value()
	if pos_tok == null:
		return
	var pos_text: String = str(pos_tok.value)
	if not pos_text.is_valid_float():
		_error("actor at 后应为数字坐标")
		return
	node.position = pos_text.to_float()
	node.has_position = true


func _token_value_to_text(token: KS_Token) -> String:
	if token.type == KS_Token.Type.VARIABLE_REF:
		return "%s%s" % [str(token.value.prefix), str(token.value.name)]
